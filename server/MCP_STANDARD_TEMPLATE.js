 ...
 * 2. Replace [SERVICE_NAME] with your service (e.g., gmail, slack, github)
...
class [SERVICE_NAME]MCP {
  constructor() {
    ...
    this.server = new Server(
      { name: '[SERVICE_NAME]-mcp', version: '1.0.0' },
      { capabilities: { tools: {} } }
    );
  ...
  setupTools() {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        {
          name: 'get_[RESOURCE]',
          description: 'Get [resource] from [service]. Use when user wants to view, check, or list [resources].',
          inputSchema: {
            type: 'object',
            properties: {
        ...
        },
        {
          ...
          description: 'Create new [resource] in [service]. Use when user wants to add, create, or make new [resource].',
          inputSchema: {
            type: 'object',
            properties: {
  ...
  async get[Resource](args) {
    ...
    try {
      ...
      const response = await nango.get({
        endpoint: '/[service]/v1/[resources]',
        connectionId: process.env.NANGO_CONNECTION_ID || 'workspace_3',
        providerConfigKey: '[service]-integration',
        params: {
          limit,
          ...(filter && { q: filter })
        }
      ...
      return {
        content: [{
          type: 'text',
          text: `Found ${items.length} [resources]:\n\n${formatted}`
        }]
      };
    } catch (error) {
      ...
      return {
        content: [{
          type: 'text',
          text: `Could not fetch [resources]. Please check your connection and try again.`
        }],
        isError: true
      };
    }
  ...
  async start() {
    ...
    console.error('[SERVICE_NAME] MCP Server started');
  }
}

// Start the server
if (require.main === module) {
  const server = new [SERVICE_NAME]MCP();
...
module.exports = [SERVICE_NAME]MCP;

/* 
IMPLEMENTATION CHECKLIST:
□ Replace all [SERVICE_NAME] with your service (e.g., Gmail)
□ Replace all [RESOURCE] with your resource (e.g., Email, Message, Issue)
