...
export function SmartConnectorBubble({ 
  ...
  const handleSync = async () => {
    ...
    setIsSyncing(true);
    try {
      await UserConnector.sync(connector.provider, { workspaceId: connector.workspaceId });
  ...
  const getStatusIcon = () => {
    if (isSyncing) return <ArrowsClockwise className="w-3 h-3 animate-spin text-white" />;
    if (status === 'error') return <Warning className="w-3 h-3 text-red-500" />;
    if (status === 'connected') return <Check className="w-3 h-3 text-green-500" />;
    ...
    >
      ...
      <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 px-3 py-2 bg-gray-900 text-white rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 whitespace-nowrap">
        ...
        <div className="text-xs text-gray-400">
          {status === 'connected' ? `Synced ${getTimeSinceSync()}` : 'Disconnected'}
        </div>
        {config.features && (
...
export function SmartConnectorAddButton({ onClick, workspace, size = 40 }) {
  ...
  useEffect(() => {
    ...
    const getSuggestions = () => {
      ...
      if (workspace?.name?.toLowerCase().includes('dev') || 
  ...
  const handleQuickAction = async (connector, action) => {
    ...
    switch (action) {
      case 'sync':
        ...
        await UserConnector.sync(connector.provider, { workspaceId: workspace.id });
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <div className="flex items-center -space-x-2">
        {connectors.slice(0, expanded ? undefined : visibleCount).map((connector, idx) => (
