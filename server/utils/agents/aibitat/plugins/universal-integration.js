    ...
    return {
      ...
      setup(aibitat) {
        ...
        aibitat.function({
          ...
          description: "Create or configure an integration with any external service. Use when user asks to connect, integrate, or sync with any service.",
          parameters: {
            type: "object",
            properties: {
              service: {
                type: "string",
                description: "Name of the service to integrate (e.g., Slack, GitHub, Salesforce)"
              },
              resources: {
                type: "array",
                description: "Types of data to sync (e.g., ['messages', 'users'] for Slack)",
                items: { type: "string" }
              },
              operations: {
                type: "array",
                description: "Operations to enable (e.g., ['create', 'update', 'delete', 'search'])",
                items: { type: "string" },
          ...
          handler: async function ({ service, resources, operations, syncFrequency, vectorFields }) {
            try {
...
- Update items: "@agent update ${service} [resource]..."`;
        ...
        aibitat.function({
          ...
          controller: new AbortController(),
          description: "Perform an action (create, update, delete) on an integrated service",
          parameters: {
            type: "object",
            properties: {
              service: {
              ...
              resource: {
                type: "string",
                description: "Resource type (e.g., 'issue', 'contact', 'event')"
              },
              data: {
                type: "object",
          ...
          handler: async function ({ service, action, resource, data }) {
            try {
              this.super.introspect(`Performing ${action} on ${service} ${resource}`);
...
async function generateNangoConfig(params) {
  const { service, resources, operations, syncFrequency, vectorFields, workspaceId } = params;
  ...
  const config = {
    ...
    actionConfig: {
      operations: operations.filter(op => ['create', 'update', 'delete'].includes(op))
    },
    vectorConfig: {
...
async function triggerInitialSync(service, workspaceId) {
  // Trigger first sync
  console.log(`[UniversalIntegration] Triggering initial sync for ${service} in workspace ${workspaceId}`);
  return true;
}
...
async function smartQuery({ service, query, resource, filters, workspaceId }) {
  ...
  if (shouldUseVector) {
    ...
    const nango = new NangoIntegration();
    return await nango.proxyQuery(service, resource, { q: query, ...filters }, workspaceId);
  }
}

async function shouldQueryVector(query) {
  // Determine if query is better suited for vector search
  const semanticKeywords = ['similar', 'like', 'about', 'related', 'concerning'];
...
function formatResults(results, service) {
  ...
  const formatted = results.slice(0, 5).map((item, i) => 
