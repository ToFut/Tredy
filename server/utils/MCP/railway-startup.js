...
async function startMCPServersForProduction() {
  ...
  try {
    ...
    if (activeServers.length > 0) {
      console.log(`[MCP Railway] Successfully started ${activeServers.length} MCP servers:`, activeServers);
    } else {
    ...
    if (process.env.NANGO_SECRET_KEY) {
      console.log("[MCP Railway] Nango integration detected - Gmail/connectors should be available");
    }
    
    if (process.env.GOOGLE_CLIENT_ID) {
      console.log("[MCP Railway] Google OAuth detected - Calendar integration should be available");
    }
