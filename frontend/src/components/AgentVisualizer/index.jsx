      ...
      <div className="glass-card rounded-2xl p-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div
              className={`w-8 h-8 rounded-full bg-gradient-to-r ${getStepColor(status)} flex items-center justify-center text-white intelligence-glow`}
            >
              {getStepIcon(status)}
            </div>
            <div>
        ...
        {operations.length > 0 && (
          <div className="space-y-2">
            {operations.map((op, index) => (
              ...
              >
                <div
                  className={`w-6 h-6 rounded-full bg-gradient-to-r ${getStepColor(op.type)} flex items-center justify-center text-white text-xs ${
                ...
                <div className="flex-1">
                  ...
                  {op.detail && (
                    <p className="text-xs text-gray-400">{op.detail}</p>
                  )}
                </div>
                {op.status === "active" && (
