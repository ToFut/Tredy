...
Only return the JSON array, no other text.`;
      ...
      try {
        ...
        console.log(
          "🔧 [WorkflowCreator] LLM parsing failed, using fallback:",
          parseError.message
        );
        return this.parseDescriptionToWorkflowStepsFallback(
          description,
          availableTools
        );
      }
      ...
      console.log(
        "🔧 [WorkflowCreator] Created",
        steps.length,
        "workflow steps via LLM"
      );
      return steps;
    } catch (error) {
      console.log(
        "🔧 [WorkflowCreator] LLM decomposition failed, using fallback:",
        error.message
      );
      return this.parseDescriptionToWorkflowStepsFallback(
        description,
        aibitat ? this.getAvailableTools(aibitat) : []
      );
    }
  }
  ...
  createWorkflowStep(stepData, index, availableTools, allSteps = []) {
    ...
    const processedParams = this.processParametersWithVariables(
      parameters || this.extractParameters(action, target, content),
      allSteps,
      index
    );
  ...
  shouldUseDirectOutput(action) {
    ...
    return directOutputActions.some((directAction) =>
      actionLower.includes(directAction)
    );
  }
  ...
  findBestTool(action, target, availableTools) {
    ...
    if (
      ...
      return availableTools.find(
        (tool) => tool.includes("gmail") && tool.includes("send_email")
      );
    }

    // LinkedIn actions
    if (
      actionLower.includes("linkedin") ||
      actionLower.includes("invite") ||
      actionLower.includes("connect")
    ) {
      if (actionLower.includes("invite") || actionLower.includes("connect")) {
        return availableTools.find(
          (tool) => tool.includes("linkedin") && tool.includes("connect")
        );
      }
      if (actionLower.includes("message")) {
        return availableTools.find(
          (tool) => tool.includes("linkedin") && tool.includes("send_message")
        );
      }
    ...
    if (
      ...
      if (actionLower.includes("book") || actionLower.includes("schedule")) {
        return availableTools.find(
          (tool) => tool.includes("calendar") && tool.includes("book")
        );
      }
    ...
    // Web scraping
    if (
      actionLower.includes("scrape") ||
      actionLower.includes("web") ||
      targetLower.includes("http")
    ) {
  ...
  extractActionsFromText(text) {
    ...
    const emailMatches = text.matchAll(
      /send\s+(?:email|mail)\s+to\s+([^\s]+@[^\s]+)\s+with\s+([^,]+?)(?:\s+and|$)/gi
    );
    ...
    const linkedinMatches = text.matchAll(
      /send\s+(?:invite|connection)\s+(?:on|in)\s+(?:linkedin|linkdin)\s+to\s+([^,]+?)(?:\s+and|$)/gi
    );
  ...
  parseGenericStep(stepText, index) {
    ...
    if (stepText.includes("@")) {
      const emailMatch = stepText.match(
  ...
  formatWorkflowPreview(workflow, steps) {
    ...
    steps.forEach((step, index) => {
      ...
      preview.push(
        `│${stepLine}${" ".repeat(Math.max(0, innerWidth - stepLine.length))}│`
      );

      if (detail) {
        const detailLine = `    └─> ${detail}`;
        preview.push(
          `│${detailLine}${" ".repeat(Math.max(0, innerWidth - detailLine.length))}│`
        );
      }
    });
    ...
    preview.push(
      `│${statusLine}${" ".repeat(Math.max(0, innerWidth - statusLine.length))}│`
    );
    preview.push("└" + "─".repeat(boxWidth) + "┘");
  ...
  async sendProgressiveUpdate(
    ...
    const progressMessage =
      `🏗️ **Building Workflow: "${draft.name}"**\n\n` +
    ...
    aibitat.introspect(
      `Progress: ${stepNumber}/${totalSteps} blocks established`
    );
      ...
      setup(aibitat) {
        ...
        function getSession(conversationId) {
          if (!sessions.has(conversationId)) {
            sessions.set(
              conversationId,
              new WorkflowCreatorSession(conversationId)
            );
          }
        ...
        aibitat.function({
          ...
          description:
            "Capture the LLM's intended tool execution sequence and convert to visual flow",
          parameters: {
            type: "object",
            properties: {
              tools: {
        ...
        console.log(
          "🔧 [WorkflowCreator] Registering create_workflow function"
        );
        console.log(
          "🔧 [WorkflowCreator] Plugin setup complete, about to register function"
        );
        aibitat.function({
          name: "create_workflow",
          description:
            "Creates automated workflows from natural language. ALWAYS use this when user describes: multiple steps, sequential tasks (then/and/after), automation requests, 'fetch and summarize', 'read and send', chained actions, or any multi-step process. Builds visual workflow that executes immediately.",
          examples: [
            {
              prompt:
                "fetch news about AI, summarize top 5 stories, and email me the summary",
              call: JSON.stringify({
                description:
                  "fetch news about AI, summarize top 5 stories, and email me the summary",
              }),
            },
            {
              ...
              call: JSON.stringify({
                description:
                  "read my last 10 emails and create a summary report",
              }),
            },
            {
              prompt:
                "check calendar then prepare meeting notes then send to attendees",
              call: JSON.stringify({
                description:
                  "check calendar then prepare meeting notes then send to attendees",
              }),
            },
            {
              ...
              call: JSON.stringify({
                description:
                  "get weather forecast and news then email daily briefing",
              }),
            },
            {
              ...
              call: JSON.stringify({
                description:
                  "analyze sales data then generate charts and send report",
              }),
            },
            {
              ...
              call: JSON.stringify({
                description:
                  "search LinkedIn profiles and compile contact list",
              }),
            },
          ],
          parameters: {
          ...
          handler: async ({ description, name }) => {
            try {
              console.log("🔧 [WorkflowCreator] Handler called with:", {
                description,
                name,
              });
              ...
              aibitat.introspect(
                `🏗️ Starting workflow creation: "${draft.name}"`
              );

              // Parse description into workflow steps using LLM-powered approach
              const parsedSteps = await session.parseDescriptionToWorkflowSteps(
                description,
                aibitat
              );
              ...
              await session.sendProgressiveUpdate(
                aibitat,
                draft,
                steps,
                1,
              ...
              for (let i = 0; i < parsedSteps.length; i++) {
                ...
                aibitat.introspect(
                  `✅ Block ${stepNum} established: ${stepTitle}`
                );
              ...
              aibitat.introspect(
                `🎉 Workflow "${workflowConfig.name}" created successfully!`
              );

              // Send final completion update
              await session.sendCompletionUpdate(
                aibitat,
                draft,
                steps,
                workflowUuid
        ...
        console.log(
          "🔧 [WorkflowCreator] create_workflow function registered successfully"
        );
        ...
        aibitat.function({
          ...
          description:
            "Simple test function to verify the workflow plugin is working. Use this when user says 'test' or 'test workflow'.",
          parameters: {
            type: "object",
            properties: {
              message: {
          ...
          handler: async ({ message }) => {
            ...
            return {
              success: true,
              message:
                "✅ Workflow plugin is working! Test successful: " + message,
            };
          },
        });
        ...
        aibitat.function({
          ...
          handler: async ({ workflowId, name }) => {
            try {
              ...
              if (result.success) {
                aibitat.introspect(
                  `Workflow "${workflowConfig.name}" saved successfully!`
                );
        ...
        aibitat.function({
          ...
          handler: async ({ workflowId, stepNumber, newDescription }) => {
            try {
              ...
              aibitat.introspect(
                `Updated step ${stepNumber}: ${newDescription}`
              );
        ...
        aibitat.function({
          name: "run_workflow",
          description:
            "Run a saved workflow by name. Use this when user says 'run [workflow name]' or 'execute [workflow name]'",
          parameters: {
            type: "object",
            properties: {
              workflowName: {
          ...
          handler: async ({ workflowName }) => {
            try {
              ...
              const flow = flows.find(
                (f) => f.name.toLowerCase() === workflowName.toLowerCase()
              );
              ...
              aibitat.introspect(
                `Workflow "${flow.name}" completed successfully`
              );
