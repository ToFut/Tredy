    ...
    <div className="w-full max-w-4xl mx-auto p-4">
      ...
      >
        {operations.map((op, index) => {
          ...
          const statusConfig =
            STATUS_CONFIG[op.status] || STATUS_CONFIG.pending;
            ...
            >
              ...
              >
                <div className="flex items-start gap-3">
                  {/* Status Icon */}
                  <div
                    className={`p-2 rounded-lg ${statusConfig.bgColor} transition-colors`}
                  >
                    <StatusIcon
                    ...
                    {op.response && (
                      <div>
                        ...
                        <pre className="text-xs bg-theme-bg-secondary rounded-lg p-3 overflow-x-auto max-h-32">
                          <code
                            className={`${op.status === "error" ? "text-red-500" : "text-theme-text-primary"}`}
                          >
                            {typeof op.response === "string"
                              ? op.response
                              : JSON.stringify(op.response, null, 2)}
