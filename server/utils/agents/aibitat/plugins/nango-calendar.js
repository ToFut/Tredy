    ...
    return {
      ...
      setup(aibitat) {
        aibitat.function({
          ...
          description: "Get calendar events for a specific date or date range. Use this when user asks about their calendar, schedule, meetings, or availability.",
          parameters: {
            type: "object",
            properties: {
              date: {
                type: "string",
                description: "Date to check in YYYY-MM-DD format. Use 'today' for current date, 'tomorrow' for next day.",
              },
              days: {
                type: "number", 
          ...
          handler: async function ({ date, days = 1 }) {
            try {
              this.super.introspect(`Checking calendar for ${date} (${days} days)`);
              ...
              console.error('[NangoCalendar] Error fetching calendar events:', error);
        ...
        aibitat.function({
          ...
          controller: new AbortController(),
          description: "Create a new calendar event. Use this when user wants to schedule meetings, appointments, or reminders.",
          parameters: {
            type: "object",
            properties: {
              title: {
          ...
          handler: async function ({ title, startTime, endTime, description, location }) {
            try {
              ...
              console.error('[NangoCalendar] Error creating calendar event:', error);
