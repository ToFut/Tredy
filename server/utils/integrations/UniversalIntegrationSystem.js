...
class UniversalIntegrationSystem {
  ...
  async discoverAPI(service, method = "auto") {
    console.log(
      `[Discovery] Discovering API for ${service} using ${method} method`
    );

    // Try different discovery methods
    const methods =
      method === "auto" ? ["openapi", "graphql", "rest", "manual"] : [method];
    ...
    for (const discoveryMethod of methods) {
      ...
      if (engine) {
        try {
          ...
          if (spec) {
            console.log(
              `[Discovery] Successfully discovered ${service} using ${discoveryMethod}`
            );
            return spec;
          }
        } catch (error) {
          console.log(
            `[Discovery] ${discoveryMethod} failed for ${service}:`,
            error.message
          );
        }
      }
  ...
  generateSyncScript(spec, config) {
    ...
    return Object.entries(syncs)
      .map(([syncName, syncConfig]) => {
    ...
    return JSON.stringify(
      {
        models: spec.models.map((model) => ({
          name: model.name,
          vectorFields: model.vectorFields || [
            "name",
            "description",
            "content",
          ],
      ...
      spec.models.forEach((model) => {
        ...
        tools.push({
          ...
          parameters: {
            id: {
              type: "string",
              description: `${model.name} ID or identifier`,
            },
          },
        });
      });
    ...
    if (spec.actions) {
      Object.entries(spec.actions).forEach(([actionName, actionConfig]) => {
        tools.push({
          ...
          parameters: this.generateSmartParameters(
            actionConfig,
            config.service
          ),
        });
        ...
        tools.push({
          ...
          parameters: {
            ...
            notify: {
              type: "boolean",
              default: true,
              description: "Show sync progress and results",
            },
          },
        });
      });
    ...
    if (service === "linkedin") {
      if (actionConfig.input === "CreatePostInput") {
        return {
          text: {
            type: "string",
            description: "Post content to share on LinkedIn",
          },
          visibility: {
            type: "string",
  ...
  async loadTemplates() {
    ...
    try {
      if (fs.existsSync(templateDir)) {
        ...
        console.log(
          `[Templates] Found template directories: ${templates.join(", ")}`
        );
        ...
        console.log(
          `[Templates] Loaded ${this.templates.size} templates: ${Array.from(this.templates.keys()).join(", ")}`
        );
      } else {
        ...
        const existingConfig =
          "/Users/segevbin/anything-llm/server/templates/integrations/linkedin/config.json";
        const newConfigPath = path.join(linkedinDir, "config.json");

        if (fs.existsSync(existingConfig)) {
          const config = JSON.parse(
            await fs.promises.readFile(existingConfig, "utf8")
          );
          ...
          console.log(
            "[Templates] Created LinkedIn template from existing config"
          );
        }
      }
    } catch (error) {
