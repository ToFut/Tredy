...
export function ThinkingIndicator({ message = "Thinking..." }) {
  ...
  return (
    <div className="flex items-center gap-2 text-theme-text-secondary text-sm py-2">
      ...
      <span className="italic">{message}{dots}</span>
    </div>
  );
}
      ...
      {steps.map((step, index) => (
        ...
        >
          <div className={`mt-0.5 ${
            ...
            {step.status === 'completed' && <CheckCircle className="w-5 h-5" weight="fill" />}
            {step.status === 'active' && <CircleNotch className="w-5 h-5 animate-spin" />}
