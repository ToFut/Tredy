...
const {
  MCPNangoBridge,
} = require("../../utils/connectors/mcp-nango-bridge.js");
      ...
      try {
        ...
        if (!process.env.NANGO_SECRET_KEY) {
          return response.status(501).json({
            success: false,
            error:
              "Connector integration not configured. Please set NANGO_SECRET_KEY in environment variables.",
          });
        }
        ...
        try {
          ...
          response.status(200).json({
            ...
            authUrl:
              authConfig.authUrl ||
              `${authConfig.host}/oauth/connect?provider_config_key=${authConfig.providerConfigKey}&connection_id=${authConfig.connectionId}&public_key=${authConfig.publicKey}`,
          ...
          return response.status(503).json({
            success: false,
            error:
              "Connector service temporarily unavailable. Please check Nango configuration.",
          });
        }
      } catch (error) {
    ...
    async (request, response) => {
      try {
        ...
        const success = response.locals.user?.id
          ? await ConnectorTokens.deleteConnector({
              userId: response.locals.user.id,
              provider,
            })
  ...
  app.post(
    ...
    async (request, response) => {
      try {
        ...
        if (connector.nangoConnectionId) {
          const syncResult = await bridge.triggerSync(
