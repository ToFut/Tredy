...
export default function SupabaseLogin() {
  ...
  const checkExistingSession = async () => {
    try {
      ...
      if (session) {
        console.log('[AUTH] Found existing Supabase session:', session.user.email);
  ...
  const syncWithBackend = async (session) => {
    try {
      ...
      console.log('[SYNC] Calling /api/auth/supabase with user:', session.user.email);
    ...
    <div className="flex items-center justify-center min-h-screen bg-theme-bg-container">
      <div className="w-full max-w-md p-8 space-y-6 bg-theme-bg-sidebar rounded-lg shadow-lg">
        ...
        <form onSubmit={handleEmailAuth} className="space-y-4">
          <div>
            <label htmlFor="email" className="block text-sm font-medium text-theme-text-primary">
              Email Address
            </label>
            <input
              id="email"
          ...
          <div>
            <label htmlFor="password" className="block text-sm font-medium text-theme-text-primary">
              Password
            </label>
            <input
              id="password"
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-theme-bg-sidebar text-theme-text-secondary">Or continue with</span>
          </div>
        </div>
