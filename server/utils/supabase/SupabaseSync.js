...
class SupabaseSync {
  ...
  async syncChat(workspaceChat) {
    ...
    try {
      const { data, error } = await this.supabase.from("chat_messages").upsert({
        id: workspaceChat.id,
  ...
  async syncDocument(document, vectorData) {
    ...
    try {
      ...
      const { data, error } = await this.supabase.from("documents").upsert({
        id: document.id,
        tenant_id: this.tenantId,
  ...
  async syncWorkspace(workspace) {
    ...
    try {
      const { data, error } = await this.supabase.from("workspaces").upsert({
        id: workspace.id,
        tenant_id: this.tenantId,
        name: workspace.name,
        slug: workspace.slug,
  ...
  async startBatchSync() {
    ...
    setInterval(async () => {
      ...
      try {
        await this.supabase.from("usage_events").insert(events);
      } catch (err) {
