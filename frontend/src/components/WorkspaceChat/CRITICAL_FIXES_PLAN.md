...
function LiveMessage({ message, workspace }) {
  ...
  return (
    <div className="message-wrapper">
      ...
      {metrics && <CompactMetricsBar tools={tools} metrics={metrics} />}
    </div>
  );
}
```
...
class InvocationQueue {
  ...
  async storeFallback(id, data) {
    ...
    await fs.writeFile(
      fallbackPath,
      JSON.stringify({ id, data, timestamp: Date.now() })
    );
  }
}
...
export default function UnifiedMessage({ message, workspace, isLive = false }) {
  ...
  return (
    <div className="unified-message">
      ...
      <div className="message-body">
        {renderContent()}
        {debugInfo.length > 0 && <DebugAccordion items={debugInfo} />}
      </div>
    </div>
  );
}
