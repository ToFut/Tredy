...
function monitoringEndpoints(app) {
   ...
   */
  app.get("/api/monitoring/health", async (request, response) => {
    try {
      ...
      const healthStatus = {
        ...
        metrics: {
          ...
          recentExecutions: {
            ...
            failed: failedExecutions,
            running: recentExecutions.filter((e) => e.status === "running")
              .length,
          },
        },
      };
      ...
      try {
        ...
        const schedulesWithStatus = await Promise.all(
          schedules.map(async (schedule) => {
            ...
            const lastExecution = await ScheduleExecution.getLastExecution(
              schedule.id
            );
        ...
        response.status(200).json({
          ...
          summary: {
            ...
            healthy: schedulesWithStatus.filter(
              (s) => s.status.health === "healthy"
            ).length,
            degraded: schedulesWithStatus.filter(
              (s) => s.status.health === "degraded"
            ).length,
            unhealthy: schedulesWithStatus.filter(
              (s) => s.status.health === "unhealthy"
            ).length,
          },
  ...
  app.get(
    ...
    async (request, response) => {
      try {
        ...
        const executionMetrics = await ScheduleExecution.getMetricsOverTime(
          parseInt(hours)
        );
        ...
        const totalExecutions = executionMetrics.reduce(
          (sum, m) => sum + m.total,
          0
        );
        const successfulExecutions = executionMetrics.reduce(
          (sum, m) => sum + m.successful,
          0
        );
        const failedExecutions = executionMetrics.reduce(
          (sum, m) => sum + m.failed,
          0
        );
        const avgDuration =
          executionMetrics.reduce((sum, m) => sum + (m.avgDuration || 0), 0) /
          executionMetrics.length;
        ...
        response.status(200).json({
          ...
          scheduleMetrics: scheduleMetrics.sort(
      ...
      try {
        ...
        for (const schedule of schedules) {
          ...
          if (lastExecution && lastExecution.status === "running") {
            const runningTime =
              Date.now() - new Date(lastExecution.started_at).getTime();
