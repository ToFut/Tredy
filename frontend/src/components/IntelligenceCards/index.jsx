...
import { Lightning, Database, Globe, ChartLine, Sparkle, ArrowsClockwise } from "@phosphor-icons/react";

export default function IntelligenceCards({ metrics = {}, insights = [], isProcessing = false }) {
          ...
          >
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-lg bg-gradient-to-r ${card.gradient} flex items-center justify-center text-white`}>
                  {card.icon}
                </div>
            ...
            {/* Card Content */}
            {card.metrics ? (
              <div className={`space-y-2 ${expandedCard === card.id ? '' : 'space-y-1'}`}>
              ...
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {card.content.slice(0, expandedCard === card.id ? 10 : 3).map((insight, idx) => (
            ...
            {expandedCard === card.id && (
              <div className="mt-4 pt-4 border-t border-white/10">
                <div className="h-32 bg-gradient-to-r from-white/5 to-white/10 rounded-lg p-3">
                  <div className="text-xs text-gray-400 mb-2">Visualization</div>
