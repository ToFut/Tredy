...
async function handleNangoWebhook(req, res) {
  try {
    ...
    const body = req.body;
    console.log("[Nango Webhook] Received:", body.type, body.model || body.providerConfigKey);
    ...
    switch (type) {
      case "sync":
        if (body.success) {
          ...
          console.error(`[Nango Webhook] Sync failed for ${providerConfigKey}:`, body.error);
          await updateSyncStatus(workspace.id, providerConfigKey, 'error', body.error);
        }
        break;
      ...
      case "auth":
        if (body.success) {
          ...
          console.error(`[Nango Webhook] Auth failed for ${providerConfigKey}:`, body.error);
        }
        break;
      ...
      case "sync:error":
        console.error(`[Nango Webhook] Sync error for ${providerConfigKey}:`, body.error);
        await updateSyncStatus(workspace.id, providerConfigKey, 'error', body.error);
        break;
...
async function handleSyncSuccess(workspace, provider, model, body) {
  ...
  try {
    ...
    if (result.next_cursor) {
      await saveLastSyncCursor(workspace.id, provider, model, result.next_cursor, recordCount, syncStatus);
    } else {
      // Even without cursor, track the sync
      await saveLastSyncCursor(workspace.id, provider, model, null, recordCount, syncStatus);
    }
    
    console.log(`[Nango Webhook] Processing ${records.length} ${model} records for ${provider}`);
  } catch (error) {
    console.error(`[Nango Webhook] Failed to fetch records:`, error);
    // Track the error in cursor system
    await saveLastSyncCursor(workspace.id, provider, model, null, 0, 'error', error.message);
    return;
  }
...
function convertToDocument(provider, model, record) {
  ...
  const baseDoc = {
    ...
    provider,
    model,
    createdAt: record.created_at || record._nango_metadata?.first_seen_at || new Date().toISOString(),
    updatedAt: record.updated_at || record._nango_metadata?.last_modified_at || new Date().toISOString(),
  };
...
async function getLastSyncCursor(workspaceId, provider, model) {
  try {
    const cursorRecord = await SyncCursors.get({ workspaceId, provider, model });
...
async function saveLastSyncCursor(workspaceId, provider, model, cursor, recordCount = 0, status = 'success', error = null) {
  try {
    ...
    console.log(`[Nango Webhook] Saved cursor for ${provider}:${model} - processed ${recordCount} records`);
  } catch (err) {
