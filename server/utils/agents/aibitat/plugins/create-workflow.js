    ...
    return {
      ...
      setup(aibitat) {
        ...
        aibitat.function({
          ...
          description:
            "Creates automated workflows from natural language. Use when user wants to create a workflow or describes multiple steps.",
          examples: [
            {
              prompt: "create a workflow to fetch weather and send email",
              call: JSON.stringify({
                description: "fetch weather and send email",
              }),
            },
          ],
          parameters: {
          ...
          handler: async function ({ description }) {
            try {
              console.log(
                "🎉 [CreateWorkflow] Handler called with:",
                description
              );
              ...
              async function parseIntoSteps(
                ...
                console.log(
                  "🚀 [CreateWorkflow] Available tools count:",
                  availableTools.length
                );
                console.log(
                  "🚀 [CreateWorkflow] Available tools:",
                  availableTools.slice(0, 10)
                );
                ...
                console.log(
                  "🔍 [CreateWorkflow] aibitat keys:",
                  aibitat ? Object.keys(aibitat) : "null"
                );
                console.log(
                  "🔍 [CreateWorkflow] aibitat.provider value:",
                  aibitat?.provider
                );
                console.log(
                  "🔍 [CreateWorkflow] aibitat.provider type:",
                  typeof aibitat?.provider
                );
                console.log(
                  "🔍 [CreateWorkflow] aibitat.defaultProvider:",
                  aibitat?.defaultProvider
                );
                console.log(
                  "🔍 [CreateWorkflow] aibitat.defaultProvider type:",
                  typeof aibitat?.defaultProvider
                );
                console.log(
                  "🔍 [CreateWorkflow] aibitat.defaultProvider keys:",
                  aibitat?.defaultProvider
                    ? Object.keys(aibitat.defaultProvider)
                    : "null"
                );
                console.log(
                  "🔍 [CreateWorkflow] aibitat.defaultProvider.complete:",
                  typeof aibitat?.defaultProvider?.complete
                );
                console.log(
                  "🔍 [CreateWorkflow] aibitat.getChatCompletion:",
                  !!aibitat?.getChatCompletion
                );
                console.log(
                  "🔍 [CreateWorkflow] aibitat.completion:",
                  !!aibitat?.completion
                );
                ...
                try {
                  ...
                  console.log(
                    "🔍 [CreateWorkflow] aibitat.provider isArray:",
                    Array.isArray(aibitat.provider)
                  );
                  if (
                    aibitat.provider &&
                    typeof aibitat.provider === "object"
                  ) {
                    console.log(
                      "🔍 [CreateWorkflow] aibitat.provider[0]:",
                      aibitat.provider[0]
                        ? Object.keys(aibitat.provider[0])
                        : "null"
                    );
                    console.log(
                      "🔍 [CreateWorkflow] aibitat.provider[0].complete:",
                      typeof aibitat.provider[0]?.complete
                    );
                    console.log(
                      "🔍 [CreateWorkflow] aibitat.provider.complete again:",
                      typeof aibitat.provider.complete
                    );
                  }

                  // Test if provider.complete exists despite being array
                  const canUseProviderComplete =
                    aibitat.provider &&
                    typeof aibitat.provider.complete === "function";
                  console.log(
                    "🔍 [CreateWorkflow] Can use provider.complete:",
                    canUseProviderComplete
                  );

                  // Test accessing first element if it's an array
                  const firstProviderComplete =
                    aibitat.provider &&
                    aibitat.provider[0] &&
                    typeof aibitat.provider[0].complete === "function";
                  console.log(
                    "🔍 [CreateWorkflow] First provider has complete:",
                    firstProviderComplete
                  );

                  providerHasComplete =
                    canUseProviderComplete || firstProviderComplete;

                  // Test if we can access it the same way simple-workflow does
                  try {
                    console.log(
                      "🔍 [CreateWorkflow] Testing provider access like simple-workflow..."
                    );
                    const testCall = aibitat.provider?.complete;
                    console.log(
                      "🔍 [CreateWorkflow] aibitat.provider?.complete:",
                      typeof testCall
                    );
                  } catch (testError) {
                    console.log(
                      "🔍 [CreateWorkflow] Provider test call failed:",
                      testError.message
                    );
                  }
                } catch (e) {
                  console.log(
                    "🔍 [CreateWorkflow] Provider complete test failed:",
                    e.message
                  );
                }
                ...
                console.log(
                  "🔍 [CreateWorkflow] hasLLMAccess final result:",
                  hasLLMAccess
                );

                if (hasMultipleActions && hasLLMAccess) {
                  console.log(
                    "🤖 [CreateWorkflow] Multi-action task detected, using AI decomposition"
                  );
                  console.log(
                    "🤖 [CreateWorkflow] aibitat available:",
                    !!aibitat
                  );
                  console.log(
                    "🤖 [CreateWorkflow] LLM access available:",
                    hasLLMAccess
                  );

                  try {
                    actions = await intelligentDecomposition(
                      desc,
                      availableTools,
                      aibitat
                    );
                    console.log(
                      `🤖 [CreateWorkflow] AI decomposition returned ${actions.length} actions`
                    );
                  } catch (error) {
                    console.log(
                      "⚠️ [CreateWorkflow] AI decomposition failed:",
                      error.message
                    );
                  }
                } else {
                  console.log(
                    "📝 [CreateWorkflow] Simple task or AI not available"
                  );
                  console.log(
                    "📝 [CreateWorkflow] hasMultipleActions:",
                    hasMultipleActions
                  );
                  console.log("📝 [CreateWorkflow] aibitat:", !!aibitat);
                  console.log(
                    "📝 [CreateWorkflow] aibitat.llm:",
                    !!aibitat?.llm
                  );
                }
                ...
                if (actions.length === 0) {
                  console.log(
                    "⚠️ [CreateWorkflow] No actions found, using fallback"
                  );
                  actions = fallbackExtraction(desc);
                }

                console.log(
                  "🚀 [CreateWorkflow] Extracted actions count:",
                  actions.length
                );
                console.log(
                  "🚀 [CreateWorkflow] Extracted actions:",
                ...
                const processedActions = actions
                  .map((action, index) => {
                    if (
                      action.type === "web" &&
                      action.action?.includes("scrape")
                    ) {
                      // Convert single web scraping action to multi-step Puppeteer workflow
                      return convertWebScrapingToPuppeteerWorkflow(
                        action,
                        index
                      );
                    }
                ...
                console.log(
                  "🚀 [CreateWorkflow] Processed actions count:",
                ...
                const steps = processedActions.map((action, index) => {
                  ...
                  if (bestTool) {
                    ...
                    params = normalizeVariableReferences(
                      params,
                      index,
                      actions
                    );
                    ...
                    const resultVar =
                      action.resultVariable || `step_${index + 1}_result`;

                    console.log(
                      `✅ [CreateWorkflow] Creating TOOL_CALL for ${bestTool} with normalized params:`,
                      params
                    );
                    console.log(
                      `✅ [CreateWorkflow] Result will be saved to: ${resultVar}`
                    );
                    ...
                    action.type === "process" ||
                    action.action?.includes("summarize") ||
                    action.action?.includes("chart")
                  ) {
                    // For processing/summarization tasks, use LLM instruction
                    console.log(
                      `📊 [CreateWorkflow] Creating LLM instruction for processing:`,
                    ...
                    if (action.parameters && action.parameters.instruction) {
                      ...
                      instruction =
                        action.text || action.action || "Process data";
                      if (action.requires && action.requires.length > 0) {
                        const requiredVars = action.requires.map(
                          (stepNum) => `{{step_${stepNum}_result}}`
                        );
                    ...
                    const resultVar =
                      action.resultVariable || `step_${index + 1}_result`;
                    console.log(
                      `📊 [CreateWorkflow] LLM instruction will save to: ${resultVar}`
                    );
                    ...
                    console.log(
                      `⚠️ [CreateWorkflow] No tool found, creating LLM instruction for:`,
                      action.text || action.action
                    );
                ...
                const visualBlocks = actions.map((action, index) => ({
                  ...
                  tool:
                    steps[index].type === "toolCall"
                      ? steps[index].config.toolName
                      : "AI Processing",
                }));

                console.log(
                  "🏁 [CreateWorkflow] Final result - Steps:",
                  steps.length,
                  "Visual blocks:",
                  visualBlocks.length
                );
              ...
              function extractSimpleActions(text) {
                ...
                const readEmailPattern =
                  /read\s+(?:my\s+)?(?:last\s+)?(\d+)?\s*email/gi;
                ...
                const summarizePattern =
                  /(?:summary|summarize|chart)[^,]*(?:urgency|action\s+items)?[^,]*/gi;
                ...
                const emailPattern =
                  /(?:send\s+)?(?:mail|email|sent)\s+to\s+([^\s]+@[^\s]+)(?:\s+with\s+([^,]+?))?(?=\s+and|\s+then|$)/gi;
                ...
                const anotherEmailPattern =
                  /send\s+another\s+(?:mail|email|invite)\s+to\s+([^\s]+@[^\s]+)/gi;
                ...
                const confirmPattern =
                  /confirm.*by\s+(?:another\s+)?(?:mail|email)/gi;
                ...
                const linkedinPattern =
                  /send\s+invite\s+(?:in|on)\s+(?:linkedin|linkdin)\s+to\s+([^,]+?)(?=\s+and|$|\s+with)/gi;
              ...
              async function intelligentDecomposition(
                desc,
                availableTools,
                aibitat
              ) {
                console.log(
                  "🧠 [IntelligentDecomposition] Breaking down complex task"
                );
...
Be INTELLIGENT - understand the user's intent, not just keywords.`;
                  ...
                  try {
                    ...
                    if (
                      ...
                      console.log(
                        "🤖 [IntelligentDecomposition] Using official aibitat.getProviderForConfig() method"
                      );
                      console.log(
                        "🤖 [IntelligentDecomposition] Default provider config:",
                        aibitat.defaultProvider
                      );
                      ...
                      console.log(
                        "🤖 [IntelligentDecomposition] Provider instance created:",
                        !!provider
                      );
                      ...
                      response =
                        completion.result || completion.content || completion;
                      console.log(
                        "🤖 [IntelligentDecomposition] LLM response received, length:",
                    ...
                    // Fallback to other methods
                      ...
                      console.log(
                        "🤖 [IntelligentDecomposition] Using getChatCompletion fallback"
                      );
                    ...
                    // Then try completion method
                      ...
                      console.log(
                        "🤖 [IntelligentDecomposition] Using completion fallback"
                      );
                    ...
                    // Finally try llm.chat if available
                      ...
                      console.log(
                        "🤖 [IntelligentDecomposition] Using llm.chat fallback"
                      );
                      ...
                      console.log(
                        "⚠️ [IntelligentDecomposition] No compatible LLM method found"
                      );
                      return [];
                    }
                  } catch (llmError) {
                    console.log(
                      "⚠️ [IntelligentDecomposition] LLM call failed:",
                      llmError.message
                    );
                    console.log(
                      "⚠️ [IntelligentDecomposition] Error details:",
                      llmError.stack
                    );
                    return [];
                  }
                  ...
                  if (!jsonMatch) {
                    console.log(
                      "⚠️ [IntelligentDecomposition] No JSON found in response"
                    );
                    return [];
                  }
                  ...
                  console.log(
                    `✅ [IntelligentDecomposition] AI extracted ${aiActions.length} actions`
                  );
                  ...
                  return aiActions.map((item, index) => ({
                    ...
                    resultVariable:
                      item.resultVariable || `step_${index + 1}_result`, // Variable name for this step's output
                    output: item.output || item.description,
                  }));
                } catch (error) {
                  console.log(
                    "⚠️ [IntelligentDecomposition] Failed:",
                    error.message
                  );
                  return [];
                }
              ...
              async function extractActionsFromText(
                ...
                console.log(
                  "🤖 [AI-ExtractActions] Using AI to decompose task:",
                  text
                );
...
Do NOT include explanations, markdown, or any other text. Only return the JSON array.`;
                  ...
                  console.log(
                    "🤖 [AI-ExtractActions] Requesting LLM decomposition..."
                  );
                  ...
                  if (!jsonMatch) {
                    console.log(
                      "⚠️ [AI-ExtractActions] No JSON array found in LLM response"
                    );
                  ...
                  console.log(
                    `✅ [AI-ExtractActions] AI extracted ${actions.length} actions:`,
                    actions
                  );
                  ...
                  console.log(
                    "⚠️ [AI-ExtractActions] LLM parsing failed:",
                    error.message
                  );
                  return fallbackExtraction(text);
                }
              ...
              function fallbackExtraction(text) {
                console.log(
                  "🔄 [FallbackExtraction] Using simple text splitting"
                );
                const parts = text
              ...
              function findBestTool(action, availableTools) {
                ...
                if (action.tool && availableTools.includes(action.tool)) {
                  console.log(
                    `✅ [ToolMapping] Using AI-suggested tool: ${action.tool}`
                  );
                  return action.tool;
                }
                ...
                console.log(
                  `🔍 [ToolMapping] Finding tool for: ${actionText} (type: ${actionType})`
                );
                ...
                if (toolScores.length > 0 && toolScores[0].score > 0) {
                  console.log(
                    `🏆 [ToolMapping] Best match: ${toolScores[0].tool} (score: ${toolScores[0].score})`
                  );
                  return toolScores[0].tool;
                }

                console.log(
                  `⚠️ [ToolMapping] No suitable tool found for: ${actionText}`
                );
                return null;
              }
              ...
              function calculateToolScore(actionText, actionType, toolName) {
                ...
                if (
                  actionType === "email" &&
                  (toolLower.includes("gmail") || toolLower.includes("mail"))
                ) {
                  score += 20;
                  if (actionText.includes("send") && toolLower.includes("send"))
                    score += 15;
                  if (
                    (actionText.includes("get") ||
                      actionText.includes("read")) &&
                ...
                if (
                  actionType === "linkedin" &&
                  toolLower.includes("linkedin")
                ) {
                  score += 20;
                  if (
                    actionText.includes("connect") &&
                    toolLower.includes("connect")
                  )
                    score += 15;
                  if (
                    actionText.includes("invite") &&
                    toolLower.includes("connect")
                  )
                    score += 10;
                }
              ...
              function normalizeVariableReferences(
                parameters,
                stepIndex,
                allActions
              ) {
                      ...
                      variableMatches.forEach((match) => {
                        ...
                        if (!varName.match(/^step_\d+_result$/)) {
                          ...
                          for (let i = 0; i < stepIndex; i++) {
                            ...
                            if (
                              varName.includes("summary") &&
                              previousAction.action?.includes("analyz")
                            ) {
                              ...
                              console.log(
                                `🔧 [VarNormalization] Mapped ${match} -> {{step_${i + 1}_result}}`
                              );
                              break;
                            } else if (
                              varName.includes("email") &&
                              previousAction.action?.includes("search")
                            ) {
                              normalizedValue = normalizedValue.replace(
                                match,
                                `{{step_${i + 1}_result}}`
              ...
              function convertWebScrapingToPuppeteerWorkflow(
                action,
                baseIndex
              ) {
              ...
              function validateToolParameters(toolName, params) {
                ...
                if (toolName === "web-scraping") {
                  ...
                  console.log(
                    `🔧 [ParamValidation] Cleaned web-scraping params:`,
                    validParams
                  );
                  return validParams;
                }
                ...
                if (toolName === "gmail-send_email") {
                  ...
                  if (cleanedParams.subject)
                    validParams.subject = cleanedParams.subject;
                  ...
                  console.log(
                    `🔧 [ParamValidation] Cleaned gmail-send_email params:`,
                    validParams
                  );
                  return validParams;
                }
                ...
                if (toolName === "gmail-get_emails") {
                  const validParams = {};
                  if (cleanedParams.num_emails)
                    validParams.num_emails = cleanedParams.num_emails;
                  if (cleanedParams.query)
                    validParams.query = cleanedParams.query;
                  console.log(
                    `🔧 [ParamValidation] Cleaned gmail-get_emails params:`,
                    validParams
                  );
                  return validParams;
                }
                ...
                if (toolName === "gcalendar-book_meeting") {
                  const validParams = {};
                  if (cleanedParams.attendees)
                    validParams.attendees = cleanedParams.attendees;
                  if (cleanedParams.start_time)
                    validParams.start_time = cleanedParams.start_time;
                  if (cleanedParams.end_time)
                    validParams.end_time = cleanedParams.end_time;
                  if (cleanedParams.description)
                    validParams.description = cleanedParams.description;
                  if (cleanedParams.title)
                    validParams.title = cleanedParams.title;
                  console.log(
                    `🔧 [ParamValidation] Cleaned gcalendar-book_meeting params:`,
                    validParams
                  );
                  return validParams;
                }
                ...
                if (toolName === "puppeteer-puppeteer_navigate") {
                  ...
                  console.log(
                    `🔧 [ParamValidation] Cleaned puppeteer-navigate params:`,
                    validParams
                  );
                  return validParams;
                }
                ...
                if (toolName === "puppeteer-puppeteer_evaluate") {
                  const validParams = {};
                  if (cleanedParams.script)
                    validParams.script = cleanedParams.script;
                  console.log(
                    `🔧 [ParamValidation] Cleaned puppeteer-evaluate params:`,
                    validParams
                  );
                  return validParams;
                }
              ...
              function extractParameters(action) {
                ...
                if (
                  ...
                  console.log(
                    `✅ [ParamExtraction] Using AI-provided parameters:`,
                    action.parameters
                  );
                ...
                if (action.type === "email") {
                  ...
                  if (action.action.includes("get")) {
                    ...
                    if (
                      ...
                      action.description?.includes("summary") ||
                      action.description?.includes("result")
                    ) {
                      ...
                      params.subject =
                        action.parameters?.subject || "Analysis Results";
                    } else {
                ...
                console.log(
                  `📝 [ParamExtraction] Extracted parameters:`,
                  params
                );
                return params;
              }
              ...
              function getStepName(action) {
                if (action.type === "email") {
                  ...
                  if (action.action.includes("confirmation"))
              ...
              function getStepDescription(action) {
                if (action.type === "email") {
                  if (action.action.includes("get"))
                    return action.text || "Get recent emails";
                  if (action.target) return `To: ${action.target}`;
                  return action.content || "Send email";
                }
                if (action.type === "linkedin")
                  return `Connect with: ${action.target}`;
                if (action.type === "process")
                  return action.content || action.text || "Process data";
              ...
              try {
                if (aibitat && aibitat.functions) {
                  availableTools = Array.from(aibitat.functions.keys())
                    .filter(
                      (name) =>
                        !name.startsWith("flow_") && name !== "create-workflow"
                    ) // Filter out workflow functions
                    .slice(0, 30); // Limit to prevent prompt bloat
                  console.log(
                    `[CreateWorkflow] Found ${availableTools.length} available tools`
                  );
                }
              } catch (error) {
                console.warn(
                  "[CreateWorkflow] Could not load available tools:",
                  error.message
                );
              }

              const { steps: workflowSteps, visualBlocks: parsedBlocks } =
                await parseIntoSteps(description, availableTools, aibitat);
              ...
              // Save initial state
              console.log(
                "💾 [CreateWorkflow] Saving initial workflow state..."
              );
              await AgentFlows.saveFlow(config.name, config, workflowUuid);
              console.log(
                "💾 [CreateWorkflow] Initial state saved successfully"
              );
              ...
              await AgentFlows.saveFlow(config.name, config, workflowUuid);
              aibitat.introspect(
                `🚀 Building workflow with ${parsedBlocks.length} steps...`
              );
              ...
              for (let i = 0; i < parsedBlocks.length; i++) {
                ...
                if (i > 0) {
                  config.visualBlocks[config.visualBlocks.length - 1].status =
                    "complete";
                }
                ...
                console.log(
                  `💾 [CreateWorkflow] Saving workflow with ${config.steps.length} steps...`
                );
                await AgentFlows.saveFlow(config.name, config, workflowUuid);
                console.log(
                  `💾 [CreateWorkflow] Saved step ${i + 1}/${parsedBlocks.length}`
                );
                aibitat.introspect(
                  `${block.icon} Adding ${block.tool}: ${block.description.substring(0, 30)}...`
                );
              ...
              console.log(
                `🏁 [CreateWorkflow] Final save - ${config.steps.length} total steps, ${parsedBlocks.length} visual blocks`
              );
              console.log(
                `🏁 [CreateWorkflow] Final config:`,
                JSON.stringify(
                  {
                    steps: config.steps.length,
                    blocks: config.visualBlocks.length,
                  },
                  null,
                  2
                )
              ...
              await AgentFlows.saveFlow(config.name, config, workflowUuid);
              console.log(
                `🏁 [CreateWorkflow] Workflow ${workflowUuid} saved successfully!`
              );
              ...
              aibitat.introspect(
                `🎉 Workflow "${workflowName}" created with ${parsedBlocks.length} steps!`
              );
