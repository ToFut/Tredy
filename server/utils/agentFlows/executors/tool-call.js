...
async function executeToolCall(config, context) {
  ...
  try {
    if (!aibitat) {
      throw new Error(
        "Agent context (aibitat) not available for tool execution"
      );
    }
    ...
    if (!fn) {
      const availableFunctions = Array.from(aibitat.functions.keys()).slice(
        0,
        10
      );
      throw new Error(
        `Tool "${toolName}" not found. Available: ${availableFunctions.join(", ")}${aibitat.functions.size > 10 ? "..." : ""}`
      );
    }
